...
public class ViewPager: NSObject {
    ...
    func scrollToIndex(_ index: Int, previousIndex: Int) {
        let activeTab = tabsViewList[index]
        let activeFrame = activeTab.frame

        let screenWidth = UIScreen.main.bounds.width
        var frameToScroll: CGRect
        let adjustment = (screenWidth - activeFrame.width) / 2
        if previousIndex >= index {
            frameToScroll = .init(x: activeFrame.minX - adjustment,
                                  y: activeFrame.minY,
        ...
        tabContainer.scrollRectToVisible(frameToScroll, animated: true)
    }
    ...
    fileprivate func setupPageViewController() {
        ...
        if !options.isEnableSwipeToScroll {
            disableSwipeToScroll()
        }
    }

    private func disableSwipeToScroll() {
    ...
    fileprivate func setupCurrentPageIndicator(currentIndex: Int, previousIndex: Int) {
        ...
        if options.isTabIndicatorAvailable {
            ...
            UIView.animate(withDuration: 0.5) {
                ...
                self.scrollToIndex(currentIndex, previousIndex: previousIndex)
    ...
    public func displayViewController(atIndex index: Int) {
        ...
        pageController?.setViewControllers([chosenViewController],
                                           direction: direction,
                                           animated: true, completion: { (isCompleted) in

                                            if isCompleted {
                                                DispatchQueue.main.async { [unowned self] in

                                                    self.pageController?.setViewControllers([chosenViewController],
                                                                                            direction: direction,
                                                                                            animated: false,
